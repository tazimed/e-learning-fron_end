...
const CreateCoursePage = () => {
  ...
  const removeChapter = (chapterId) => setCourseData((prev) => ({ ...prev, chapitres: prev.chapitres.filter((c) => c.id !== chapterId) }));
  const addLesson = (chapterId) => setCourseData((prev) => ({ ...prev, chapitres: prev.chapitres.map((c) => c.id === chapterId ? { ...c, lecons: [...c.lecons, { id: "temp-" + Date.now(), titre: "", contenu: "", video_url: "", ressources: [] }] } : c) }));
  const removeLesson = (chapterId, lessonId) => setCourseData((prev) => ({ ...prev, chapitres: prev.chapitres.map((c) => c.id === chapterId ? { ...c, lecons: c.lecons.filter((l) => l.id !== lessonId) } : c) }));

  const addResource = (chapterId, lessonId, url, nom, type = null) => {
    const finalType = type || (url.endsWith(".pdf") ? "pdf" : "image");
    setCourseData((prev) => ({
      ...prev,
      chapitres: prev.chapitres.map((c) => c.id === chapterId ? { ...c, lecons: c.lecons.map((l) => l.id === lessonId ? { ...l, ressources: [...l.ressources, { nom, type: finalType, url }] } : l) } : c),
    }));
  };

  const removeResource = (chapterId, lessonId, resIndex) => setCourseData((prev) => ({ ...prev, chapitres: prev.chapitres.map((c) => c.id === chapterId ? { ...c, lecons: c.lecons.map((l) => l.id === lessonId ? { ...l, ressources: l.ressources.filter((_, i) => i !== resIndex) } : l) } : c) }));
          ...
          <div className="form-step animate-in">
            ...
            {!courseData.hasPrerequisites ? (
              ...
              >
                ...
                <p>
                  Les étudiants pourront accéder directement à ce cours.
                </p>
              ...
              <div className="course-grid">
                ...
                {availableCourses
                  ...
                  .map((course) => {
                    ...
                    const selected =
                      courseData.prerequisites.includes(course.id);
        ...
        {step === 4 && (
          <div className="form-step animate-in text-center">
            ...
            <div className="summary-card">
              ...
              <p>
                Votre cours contient{" "}
                {courseData.chapitres.length} chapitres.
              </p>
