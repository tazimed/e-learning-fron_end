      ...
      <div className="courses-dashboard-grid">
        {courses.length > 0 ? (
            ...
            <div key={course.id} className="teacher-course-card glass-card">
              <div className="course-card-top">
                ...
                <div className="course-info-mini">
                  <h3>{course.title}</h3>
                  <span className="badge-meta">{course.level}</span>
                </div>
                <div className="course-status-badge">
                  {course.is_active ? (
                    <span className="status-online"><Eye size={14} /> Public</span>
                  ) : (
                    <span className="status-offline"><EyeOff size={14} /> Masqué</span>
                  )}
                </div>
              </div>
              ...
              <div className="course-stats-row">
                <div className="stat">
                  <Users size={16} />
                  <span><strong>{course.students_count}</strong> étudiants</span>
                </div>
                <div className="stat">
                  <BookOpen size={16} />
                  <span><strong>{course.chapters_count}</strong> chapitres</span>
                </div>
                <div className="stat">
                  <Calendar size={16} />
              ...
              <div className="course-actions-footer">
                <Link to={`/teacher/edit-course/${course.id}`} className="btn-action">
                ...
                >
                  {course.is_active ? <><EyeOff size={18} /> Masquer</> : <><Eye size={18} /> Afficher</>}
                </button>
          ...
          <div className="empty-state">
            ...
            <Link to="/teacher/create-course" className="btn-primary mt-4">Créer mon premier cours</Link>
          </div>
        )}
      </div>
    </div>
